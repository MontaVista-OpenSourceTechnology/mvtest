...
def pytest_configure(config):
    ...
    if machine_file_list:
        ...
        match = re.findall(r'''MACHINE.*["'](.*)["']''', machine_file_content, re.M)
        if match:
            machine = match[0]
        else:
            machine = 'Unknown'
    ...
    if msd_version == 'Unknown':
        ...
        if msd_ver_file_list:
            ...
            msd_ver_file_output = run_cmd('cat %s' % msd_ver_file, check_rc=False)
            match = re.findall(r'MSD.*VERSION="(.*)"', msd_ver_file_output, re.M)
            if match:
                msd_version = match[0]
